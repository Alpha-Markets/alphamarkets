...
- **Logo (2026-09-21):** the ribbon mark supplied by the product owner. The master is `apps/web/src/assets/alpha-market-logo.svg` (a 2000 px raster and its alpha mask wrapped in an SVG, so 985 KB: too heavy to serve, and not scalable), from which `alphamarkets-mark.png` (480 px wide, trimmed, transparent) is cut. To change the mark, replace the SVG and regenerate the PNG and the favicons from it. The landing hero shows the logo centred above the headline, breathing between a few shapes of itself (`MorphingLogo.tsx`). The supplied logo is a raster, so `apps/web/scripts/build-logo-paths.py` (Pillow, numpy, potrace) rebuilds it as 34 stacked vector layers, each traced from a lightness band and filled with a gradient fitted to its own pixels, into `src/lib/logo-paths.json` (69 KB, about 24 KB gzipped); it looks like the original with faint banding. The morph states are not new drawings: `src/lib/logo-morph.ts` moves the same points with a few soft fields (foot swells, arch rises, tail swells, settle; `STRENGTH` is the one knob for how bold), recentred, so every state has identical path structure and GSAP's MorphSVG plugin (free in gsap 3.15) interpolates each layer point to point; `map: "position"` is set because the default size matching swapped near-equal islands mid-morph in one step. The loop is rest, four states, rest at 1.7 s a step (about 8.5 s), `sine.inOut`; nothing rotates or bounces. The server renders the mark at rest; GSAP, MorphSVG and the state data are a separate lazy chunk (about 30 KB gzip) loaded after first paint and never loaded with reduced motion, where the mark stays at rest. The loop pauses off screen. The reference video (a flat blob going round, peanut, bow-tie) set the rhythm: swell, pinch, swell the other way. Measured in headless Chrome without a GPU: about 10% of the main thread; raster-bound at about 25 fps there, which a GPU browser should beat. Reuse it anywhere as `<MorphingLogo className="w-64" />` (props: `title`, `secondsPerState`, `paused`, `idPrefix`). The header shows the mark without the wordmark below 1024 px, where the navigation and wallet button need the room is in the header and footer (`Logo.tsx`), the favicon and Apple icon (`icon.png`, `apple-icon.png`) and the social preview. It is the only teal on the site; the rest stays charcoal and off-white. **Open follow-up, not blocking:** explorer labels cannot be set from this repo (they depend on explorer verification, which is blocked).
...
- Explorer verification: done for `1.4.0-testnet` (see Hosting).
...
- **Markets (2026-09-21).** Testnet lists five markets: NVDA, TSLA, AAPL, META and HOOD (the brief's Section 1 examples). NVDA is seeded by `ConfigureMarkets.s.sol`; the other four by `AddMarket.s.sol` (parameters and addresses in `packages/contracts/CHANGELOG.md`). Each has a mock token and a mock price feed the keeper keeps fresh, with placeholder prices, risk limits and fees. Adding them changed no service or frontend code, which is the "config over redesign" test from Engineering Practices. MSFT, AMZN, GOOGL, COIN, MSTR, SPY and QQQ from Section 5 are not listed: add one with `AddMarket.s.sol` when product wants it.
