...
- **Explorer verification, retried 2026-09-21:** still blocked. `forge verify-contract` fails with an expired certificate on `explorer.testnet.chain.robinhood.com`, and on some attempts the host presents a certificate for an unrelated domain. Re-run `CHAIN_ID=46630 bash script/verify.sh` from `packages/contracts` when the explorer is fixed.
