...
function CopyIcon({ copied }: { copied: boolean }) {
  return copied ? (
    <svg aria-hidden="true" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.4" className="size-3 shrink-0">
      <path d="M2.5 6.5l2.4 2.4L9.5 3.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ) : (
    <svg aria-hidden="true" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2" className="size-3 shrink-0">
  ...
  return (
    <div className={cn("inline-flex max-w-full flex-wrap items-center gap-2 rounded-control border border-line bg-surface px-3 py-1.5", className)}>
      ...
      <button
        ...
        className={cn("shrink-0 rounded-control p-0.5 text-muted transition-colors duration-150 hover:bg-accent-soft hover:text-accent", copied && "text-up")}
      >
        <CopyIcon copied={copied} />
      </button>
      {url ? (
        <a href={url} target="_blank" rel="noreferrer" aria-label="View CA on the explorer" className="shrink-0 rounded-control p-0.5 text-muted transition-colors duration-150 hover:bg-accent-soft hover:text-accent">
