...
/// 1024 px, where the navigation and the wallet button need the room; the link around it has its own aria-label.
export function Logo({ size = "md" }: { size?: "md" | "lg" }) {
  return (
    <span className="inline-flex items-center gap-3">
      <Image src={mark} alt="" priority className={size === "lg" ? "h-10 w-auto" : "h-8 w-auto"} />
      <span className={size === "lg" ? "text-sm font-medium tracking-[0.32em]" : "hidden text-sm font-medium tracking-[0.32em] lg:inline"}>ALPHAMARKETS</span>
    </span>
  );
}
