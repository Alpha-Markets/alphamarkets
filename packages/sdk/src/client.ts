...
import { chains, resolveAddresses, type ChainId, type ContractAddresses } from "@alphamarkets/config";
...
export interface AlphaMarketsConfig {
  chainId: ChainId;
  ...
  /// with any overrides from the `ALPHAMARKETS_ADDRESSES` environment variable (see `resolveAddresses`).
...
function buildClient(config: AlphaMarketsConfig) {
...
export type AlphaMarketsClient = ReturnType<typeof buildClient>;

/// `simulateContract`'s `request` result carries an `account` typed against this client's
/// account generic, which is `Account | undefined` here since `AlphaMarketsConfig.account` is
...
export function sendTransaction(client: AlphaMarketsClient, request: object) {
  return client.writeContract(request as Parameters<AlphaMarketsClient["writeContract"]>[0]);
}

export class AlphaMarkets {
  ...
  constructor(config: AlphaMarketsConfig) {
